...
if (!$smarty->isCached('list.html', $cache_id))
{
	...
	$sql='SELECT b.blog_id,b.title,b.description,'.$select_content.'b.add_time,b.views,b.comments,b.password,b.view_group,b.url_type,u.user_name,c.cat_name,c.cat_id,c.url_type as cat_url_type  FROM '.table('blog').
	...
	if ($blog_list=$db->getall($sql))
	{
		foreach ($blog_list as $key=>$val)
		{
			...
			if ($val['password'])
			{
				if ($group_id!=1)
				{
					$domain=$config['domain'].'blog.php?id='.$id;
					$blog_list[$key]['description']=<<<DTD
					<form  name="form1" method="post" action="$domain">
  					请输入查看密码：
