...
function make_sidebar()
{
	if (SLSYS=='NORMAL') {
		...
		{
			$sidebar_html.="<div class=\"$sidebar[module_id] sidebar_all\">";
			$sidebar_html.="<div class=\"c_1 c1_all\"><h2>$sidebar[title]</h2> </div> \n<div class=\"c_2 c2_all\">\n";
			$sidebar_html.=htmlspecialchars_decode($sidebar['content']);
			$sidebar_html.='</div><div class="c_3 c3_all"></div></div>';
		}
		$sidebar_html.='</div>';
...
function clear_tpl($flush_mmc1=0)
{
	...
	else
	{
		...
		$GLOBALS['smarty']->cache_dir      = PBBLOG_ROOT . 'home/cache';
		$GLOBALS['smarty']->compile_dir=PBBLOG_ROOT.'/home/compiled';
		$GLOBALS['smarty']->clearAllCache();
		$GLOBALS['smarty']->clearCompiledTemplate();
		$GLOBALS['smarty']->compile_dir=PBBLOG_ROOT.'/home/admin_compiled';
		$GLOBALS['smarty']->clearCompiledTemplate();
