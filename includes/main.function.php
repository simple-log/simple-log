...
function assign_page_info($title='',$key='',$desc='')
{
	...
	$GLOBALS['smarty']->assign('icp_no',$GLOBALS['config']['icp_no']);
	$GLOBALS['smarty']->assign('blog_notice',$GLOBALS['config']['notice']);
...
function insert_member_info()
{
	...
	if ($_SESSION['user_id'] > 0)
	{
		...
		if ($user['group_id']==1)
		{
			$admin_url='<li><a href="'.$domain.PBBLOG_WS_ADMIN.'"/>后台管理</a></li>';
		}
...
function sl_config()
{
	if (SLSYS=='NORMAL')
	{
		if (file_exists(PBBLOG_ROOT.'/home/cache/sl_config.php'))
		{
			$sc_tmp=file_get_contents(PBBLOG_ROOT.'/home/cache/sl_config.php');
			$config=unserialize($sc_tmp);
		}
		else
		{
			$config=get_sl_config();
			$fp=@fopen(PBBLOG_ROOT.'home/cache/sl_config.php',"w");
			flock($fp,LOCK_EX);
	...
	else
	{
		if (defined('MCINIT'))
		{
			$config= memcache_get($GLOBALS['mmc'],$_SERVER['HTTP_APPVERSION']."mcconfig");
			if (empty($config))
			{
				$config=get_sl_config();
				memcache_set($GLOBALS['mmc'],$_SERVER['HTTP_APPVERSION']."mcconfig",$config);
			}
		}
		else
		{
