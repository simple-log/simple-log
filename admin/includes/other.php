...
elseif ($action=='del_attachment')
{
	...
	$file_name=$db->getone('SELECT file_name FROM '.table('attachments')." WHERE attachment_id='".$attachment_id."'");
	if (empty($file_name))
	{
		sys_message('附件不存在',$referer_url);
	}
	...
	if ($master_db?$db2->query($sql):$db->query($sql))
	{
		...
		{
			$s = new SaeStorage();
			$s->delete('simplelog',$file_name);
		}
		else
		{
			unlink(PBBLOG_ROOT.'/'.$file_name);
		}
...
elseif ($action=='auto_save')
{
	...
	$sql=('DELETE FROM '.table('page')." WHERE relate_id='".$id."'". " AND  user_id='".$user_id."' AND type='-1'");
	...
	if (($master_db?$db2->query($sql):$db->query($sql))===false)
	{
